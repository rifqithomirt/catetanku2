requirejs(['/assets/js/uuid.min.js'],
    function (uuid) {
        $(document).ready(async function () {
            var xml = await R.agetFile('/pages/vendor/index.xml', '');
            $('#content-body-section').html(xml);
            var sql = `SELECT (catetanku.vendors.id) AS _id, column_json(catetanku.vendors.doc) AS doc
            FROM catetanku.vendors`;
            ...
            $(function () {
                ...
                if (("laravel" === $("body").attr("data-framework") && (r = $("body").attr("data-asset-path")), e.length)) {
                    ...
                    $("div.head-label").html('<h6 class="mb-0">Vendors</h6>');
                }
