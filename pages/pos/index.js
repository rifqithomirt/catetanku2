requirejs(['/assets/js/uuid.min.js'],
  function (uuid, mask) {
    $(document).ready(async function () {
      ...
      LIMIT 1000`;
      var resQueryCategories = await R.aget(`${R.protocol}://${R.host}:${R.port}/webapi/query/?sql=${ encodeURIComponent(sql)}`, '');
      ...
      var resQueryTransactions = await R.aget(`${R.protocol}://${R.host}:${R.port}/webapi/query/?sql=${ encodeURIComponent(sql)}`, '');
      ...
      $('#submit').on('click', async function(){
        ...
        await R.asyncForEach(arrayDataCart, async function( obj ){
          var uid = uuid.v1();
          var res = await R.apost(`${R.protocol}://${R.host}:${R.port}/webapi/transactions/${uid}`, '', obj);
          console.log(res);
        });
        $.alert({
          'title': 'Alert!',
